...
class ShipperAvailableOrderListView(generics.ListAPIView):
	...
	def list(self, request, *args, **kwargs):
		if not is_shipper(request.user): 
			return Response({'detail': 'Only shippers can view this queue.'}, status=403)

		if not settings.GHTK_MOCK_MODE: 
			return mock_mode_error()
...
class ShipperMyOrderListView(generics.ListAPIView):
	...
	def list(self, request, *args, **kwargs):
		if not is_shipper(request.user): 
			return Response({'detail': 'Only shippers can view deliveries.'}, status=403)
...
class ShipperOrderDetailView(generics.RetrieveAPIView):
	...
	def retrieve(self, request, *args, **kwargs):
		if not is_shipper(request.user): 
			return Response({'detail': 'Only shippers can view this delivery.'}, status=403)
...
class ShipperClaimOrderView(APIView):
	...
	def post(self, request, medicine_order_id):
		if not is_shipper(request.user): 
			return Response({'detail': 'Only shippers can claim deliveries.'}, status=403)
		if not settings.GHTK_MOCK_MODE: 
			return mock_mode_error()
...
class ShipperStatusUpdateView(APIView):
	...
	def patch(self, request, medicine_order_id):
		if not is_shipper(request.user): 
			return Response({'detail': 'Only shippers can update deliveries.'}, status=403)
